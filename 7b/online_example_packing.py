...
class ExamplePackDataset(IterableDataset):
    def __init__(self, dataset, tokenizer, batch_size, max_seq_length, partition, max_batch, seed, shard=False):
        ...
        if shard:
            ...
            self.dataset_indices = list(range(len(self.dataset)))        # total number of samples to be processed by 1 gpu
        self.length = len(self.dataset_indices)
        # batch size in terms of number of tokens
        self.batch_size_token = batch_size * self.max_seq_length
    ...
    def load_dataset_states(self, output_dir):
