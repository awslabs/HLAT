...
if [ $1 = "compile" ]; then
    if [ $LOCAL_CACHE_ENABLED -gt 0 ]; then
        ...
        neuron_parallel_compile bash $SCRIPT_DIR/tp_zero1_llama2_7b_hf_pretrain.sh $TASK 3>&1 2>&1 | tee $OUTPUT_LOG
    else
...
if [ $1 = "run" ]; then
    ...
    bash $SCRIPT_DIR/tp_zero1_llama2_7b_hf_pretrain.sh $TASK 3>&1 2>&1 | tee $OUTPUT_LOG
fi
