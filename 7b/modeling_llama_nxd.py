...
def _make_causal_mask(
    ...
    # the condition is tril including the diagonal
    # mask is low tri (including diagonal) is 0, upper tri is -inf
    ...
    def forward(self, x):
        if self.pretraining_tp > 1:
            ...
            if self.config.split_linear:
                ...
                # Haozheng: changes for coalesced linear
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if self.num_heads == self.num_key_value_heads and not self.config.split_linear:
    ...
    def forward(
        ...
        def core_attn(query_states, key_states, value_states):
            ...
            # Haozheng: attention_mask optimization
            # In attention_mask, low tri (including diagonal) is 0 and upper tri is -inf
            if not self.config.constant_attention_mask:
        ...
        # Haozheng: selective checkpoint
        # xm.master_print(f"selective_checkpoint_enabled = {self.config.selective_checkpoint_enabled}")
        if self.config.selective_checkpoint_enabled:
            #import torch_xla.utils.checkpoint as xla_checkpoint
            #attn_output = xla_checkpoint.checkpoint(core_attn, query_states, key_states, value_states)
            # Mason debug for checkpoint
            attn_output = torch.utils.checkpoint.checkpoint(core_attn, query_states, key_states, value_states)
        else:
            attn_output = core_attn(query_states, key_states, value_states)
        ...
        # Haozheng: sequence parallel
        if self.config.sequence_parallel_enabled:
            # attn_output is (bsz, num_heads, q_len, self.head_dim)
            attn_output = attn_output.permute(2, 0, 1, 3)
            # attn_output is (q_len, bsz, num_heads, self.head_dim)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        # Haozheng: attention_mask is always ones
        if attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            # Haozheng: expanded_attn_mask is always zeros
    ...
    def forward(
        ...
        # Haozheng: sequence parallel:
        if self.config.sequence_parallel_enabled:
            # to_model_parallel must be False
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if self.config.dataloader_type == "nxd":
                # Shift so that tokens < n predict n
                shift_logits = logits[..., :-1, :].contiguous()
                shift_labels = labels[..., 1:].contiguous()
            else:
                assert self.config.dataloader_type == "nemo"
                shift_logits = logits
                shift_labels = labels
            # Flatten the tokens
            # Mason changed for TP
