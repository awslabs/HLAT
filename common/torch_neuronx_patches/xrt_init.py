...
XRT_SERVER_REGEX = "torch_neuronx.distributed._xrt_run_server"
...
def _setup_workers(world_size, rank, local_world_size, local_rank, node_list):
    # Set up workers across nodes. xmp.spawn() does this locally
    # by figuring out free ports on the node.
    # We do this globally by doing an allgather of locally obtained
    ...
    my_worker = "{}:{};grpc://{}:{}".format(
        "localservice", rank, host, ports[local_rank]
    )
    all_workers = []
...
def _get_address_from_store(key, rank):
    if rank == 0:
        # We are hardcoding the port number here and making sure we are
        # picking a port outside of ephemeral range. We have seen issue
        # when we let the socket return a free port. It may return a port
        # from ephemeral range resulting in unexplained behaviors.
        port = "62182" if key == "nccl_info" else get_free_tcp_ports()[0]
        try:
            host = socket.gethostbyname(socket.getfqdn())
        # If no ipv4 address associated hostname, get ipv6 address
        except socket.gaierror:
            host = '[' + socket.getaddrinfo(socket.getfqdn(), None, socket.AF_INET6)[0][4][0] + ']'
...
def _set_tpu_xrt_envs(local_rank, rank, group_rank, local_world_size, world_size):
    total_nodes = world_size // local_world_size

    sleep_second = rank / local_world_size
    print(f"rank {rank} sleep {sleep_second} seconds to avoid overload TCPStore server")
    ...
    os.environ[xenv.LOCAL_WORKER] = (
        f"localservice:{group_rank}"
        if total_nodes == 1
        else f"c_localservice:{group_rank}"
    )
    ...
    if not os.environ.get("TF_GRPC_DEFAULT_OPTIONS", None):
        os.environ["TF_GRPC_DEFAULT_OPTIONS"] = (
            ...
            "grpc.http2.max_pings_without_data=0,grpc.http2.min_ping_interval_without_data_ms=300000"  # noqa: E501
        )
...
def _init_xrt_context(master_addr=None, master_port=None, store=None):
    """
    Initializes the XLA device depending on the kind of the device.
    Or is a no-op if init_xrt_context has already been called.
    ...
    try:
        _, dev_kind = _get_devices_per_worker()
    except RuntimeError:
        # Defaulting to TPU if no env variable is set
        dev_kind = "TPU"
    ...
    if store is None:
        ...
        _TCP_STORE = dist.TCPStore(master_addr, int(master_port), world_size, is_server)
    else:
        _TCP_STORE = store
    ...
    if dev_kind == "TPU":
        tpu_config_port = _set_tpu_xrt_envs(
            local_rank, rank, group_rank, local_world_size, world_size
        )
    elif dev_kind == "GPU":
        set_xrt_envs(dev_kind, world_size, rank, local_rank)
        _create_devices(dev_kind, world_size)
        _setup_workers(world_size, rank, False, node_list)

    _set_mesh_config(rank)

    # TODO: Currently, we are using the TPU flow. Once all variables are
    # renamed to NEURON, replace the TPU check with NEURON
    if dev_kind == "TPU":
        _setup_nccl_service(rank)
        _set_neuron_envs(rank, world_size, local_world_size)

        total_nodes = world_size // local_world_size  # noqa: F841
        if local_rank == 0:
